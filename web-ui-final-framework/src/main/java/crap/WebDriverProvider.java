...
class WebDriverProvider {
    ...
    private static Logger LOG = LoggerFactory.getLogger(WebDriverProvider.class);

    static WebDriver createDriverWith(Browser browser) throws MalformedURLException {
    ...
    private static WebDriver createDriverWith(DesiredCapabilities capabilities) throws MalformedURLException {
        setProxySettings(capabilities);
        LOG.info("Creating RemoteWebDriver instance...");
        try {
            WebDriver driver;
            if (ANDROID.equals(capabilities.getPlatform())) {
                ...
                LOG.info("Setting up AndroidDriver timeout...");
                ...
                LOG.info("Setting up WebDriver timeouts...");
                driver.manage().timeouts().implicitlyWait(CONFIGURATION.WebDriver().getImplicitTimeOut(), SECONDS);
    ...
    private static void setProxySettings(DesiredCapabilities capabilities) {
        // Set proxy settings, if any
        if (CONFIGURATION.Proxy().isEnabled()) {
            LOG.info("Setting WebDriver proxy...");
    ...
    private static WebDriver setListeners(WebDriver driver) {
        if (CONFIGURATION.WebDriver().isUseListener()) {
            LOG.info("Setting up WebDriver listener...");
